...
const getRandomNumber = () => {  //random yhteenkertaan
 ...
 //  setCount(count + 1);
   ...
   console.log(number);
    if (anumber > number ){
      Alert.alert('Your guess: ' + anumber + ' is too high '); 
    
    } else if (anumber < number){
      Alert.alert('Your guess: ' + anumber + ' is too low ' ); 
         
    } else if (anumber == number){
      Alert.alert('Congratulations! Your guess is correct number : ' + anumber +  ' at ' +lkm+ ' attempt'); 
      //setCount(1);
      lkm=0;
      getRandomNumber();
    }
